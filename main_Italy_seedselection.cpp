...
//    	reduced_greedyset
		...
		for(int timeBatch = 0; timeBatch<t2;timeBatch++)
		{
			...
			double influence_A = DTIC(t1, t2,reduced_greedy_batch, GT, ntrials);
...
//	    	vector<int> v = new_DTIC3(t1, t2, S, RSM, ESM,MaxDegree,IterrandomMethod, G);
	    	nintersect = v[0];
	    	...
	    	for(int dynamicIter = 3; dynamicIter<t2; dynamicIter++)
			{
