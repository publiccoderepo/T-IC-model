...
DiGraph * readDiGraph_basic(string path, double p, int nInterval);
DiGraph * readDiGraph_a(string path, int nInterval);
DiGraph * readDiGraph_b(string path, int nInterval);
...
//	map<int,unordered_set<int>> greedy_Kset_map = greedy_solution_set(NODE_NUM, largestK, batch_K, 10000,  t1, t2, G);
//	map<int, map<int,unordered_set<int>>> reduced_greedyset = longgreedy_solution_set(NODE_NUM, largestK, batch_K, 100,  t1, t2, G);
...
//    	    unordered_set<int> A_val = *it_A;
		...
		fprintf(fppoisResult, "TIMMU_batch pois unique size %d:\n", poisSet_A.size());
		for (unordered_set<int>::iterator w = poisSet_A.begin(); w != poisSet_A.end(); w++) {
			int poisVal = *w;
			fprintf(fppoisResult, "%d, ", poisVal);
		}
		...
		for(int timeBatch = 0; timeBatch<t2;timeBatch++)
		{
			...
			unordered_set<int> poisSet_A;
			for (auto e : dynamic_batch) {
				fprintf(fppoisResult, "dynamic --- People Node---time = %d, %d:", timeBatch, e);
				unordered_set<int> readPois = nodesPoisList[e];
				for (unordered_set<int>::iterator w = readPois.begin(); w != readPois.end(); w++) {
					int poisVal = *w;
					fprintf(fppoisResult, "%d, ", poisVal);
			...
			fprintf(fppoisResult, "dynamic pois unique size time = %d, %d:\n", timeBatch, poisSet_A.size());
			for (unordered_set<int>::iterator w = poisSet_A.begin(); w != poisSet_A.end(); w++) {
				int poisVal = *w;
				fprintf(fppoisResult, "%d, ", poisVal);
			}
...
//				printf("");
	    ...
	    for (int readRand = 1; readRand<=randomIterNum; readRand++)
	    {
	    	...
	    	for (auto e : *randomRound){
	    		unordered_set<int> readPois = nodesPoisList[e];
	    		fprintf(fppoisResult, "Random --- People Node---%d:", e);
	    		for (unordered_set<int>::iterator w = readPois.begin(); w != readPois.end(); w++) {
	    			int poisVal = *w;
	    			fprintf(fppoisResult, "%d, ", poisVal);
	    	...
	    	fprintf(fppoisResult, "randomRound pois unique size readRand = %d, %d:\n", readRand, poisSet_Random.size());
	    	for (unordered_set<int>::iterator w = poisSet_Random.begin(); w != poisSet_Random.end(); w++) {
				int poisVal = *w;
				fprintf(fppoisResult, "%d, ", poisVal);
			}
			fprintf(fppoisResult, "\n");
...
//, TIMMU_batch,dynamic_batchsize_list,
...
//	    printf("readDiGraph_overall batchNum = %d, batch_K = %d \n", batchNum, batch_K);
...
//	    	 printf(" the i = %d \n",i);
	        ...
	        //	max_K_degree[mapID] = tempNodesList;
	     }

	       // all_random_K_nodes randomIterNum
//	    printf("get random list!, randomIterNum = %d, batchNum = %d, \n", randomIterNum, batchNum);
