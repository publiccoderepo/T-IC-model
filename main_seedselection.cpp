...
int main(int argc, char** argv) {
	...
	map<int, map<int,unordered_set<int>>> reduced_greedyset = longgreedy_solution_set(NODE_NUM, largestK, batch_K, 10000,  t1, t2, G);
	printf("process reduced_greedyset.size()=%d", reduced_greedyset.size());
	map<int, map<int,unordered_set<int>>>::iterator iter1;


    omp_set_num_threads(100);

    int batchKNum = largestK/batch_K;
    string summaryResult = "./experiment_result/"+ runMode + "/summary_" + dataset_name + "_time_window_" + to_string(t2) + to_string(iterNum) + ".txt";
...
//    	reduced_greedyset
    	double reduced_greedy_average_influence = 0.0;
		map<int, unordered_set<int>> reduced_greedy_batchsize_list;
		for(int timeBatch = 0; timeBatch<t2;timeBatch++)
		{
			map<int, unordered_set<int>> tempIM_set = reduced_greedyset[timeBatch];
			for(int batchIter = 1; batchIter <=(i+1); batchIter++)
			{
				unordered_set<int> temp = tempIM_set[batchIter*batch_K];
				reduced_greedy_batch.insert(temp.begin(), temp.end());
			}

			double influence_A = DTIC(timeBatch, timeBatch, reduced_greedy_batch, GT, ntrials);
			reduced_greedy_average_influence = reduced_greedy_average_influence + influence_A;
			reduced_greedy_batchsize_list[timeBatch] = reduced_greedy_batch;
			reduced_greedy_batch.clear();
		}
		reduced_greedy_average_influence = reduced_greedy_average_influence/t2;

		greedy_content_influence = greedy_content_influence + to_string(reduced_greedy_average_influence) + ",";
		printf("greedy influence %f\n", reduced_greedy_average_influence);
		printf("greedy Num %d\n", reduced_greedy_batch.size());


		for(int randNum = 1; randNum <= randomIterNum; randNum++)
		{
			printf("Here random randNum = %d \n", randNum);
			unordered_set<int> tempRandomList;
...
//				printf("");
	    ...
	    int nintersect1 = 0, nintersect2 = 0, nintersect3 = 0, nintersect4 = 0,nintersect5 = 0,
	    		binary1 = 0, binary2 = 0, binary3 = 0, binary4 = 0, binary5 = 0;
	    double detection_rate1 = 0.0, detection_rate2 = 0.0, detection_rate3 = 0.0, detection_rate4 = 0.0, detection_rate5 = 0.0;
	    ...
	    for (int iA = 0; iA < ntrials; iA++) {
	    	...
	    	int nintersect;
	    	vector<int> v = greedy_DTIC(t1, t2,S, RSM, ESM,MaxDegree, reduced_greedy_batchsize_list, IterrandomMethod, G);
	    	...
	    	for(int dynamicIter = 3; dynamicIter<3+t2; dynamicIter++)
			{
	    ...
	    random_content_expected_vertices = random_content_expected_vertices + to_string((double) nintersect5 / ntrials) + ",";
	    ...
	    random_content_binary_success = random_content_binary_success + to_string((double) binary5 / ntrials) + ",";
    }
